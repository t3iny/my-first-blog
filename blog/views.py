...
def post_list(request):  # Отвечает за отображение списка опубликованных постов блога.
    ...
    # Возвращяем отрендеринный шаблон blog/post_list.html, передавая ему список постов. Request  — это объект
...
def post_new(request):  # Функции обработки GET-запросы, когда пользователь заходит на страницу создания нового поста.
    form = PostForm()   # Создает пустую форму PostForm тобиш это класс прописанный в forms.py и связана с моделью Post.
...
def post_new_post(request):  # А тут уже POST-запросы и GET-запросы. Тобиш отправка формы и создания нового поста.
    ...
    else:  # Если не является, проверка валидности не выполняется.
        form = PostForm()  # Создает пустую форму, аналогично def post_new(request).
    return render(request, 'blog/post_edit.html', {'form': form})  # Тут понятно.
...
def post_edit(request, pk):  # Функция принимает 2 аргумента: request - HTTP-запрос пользователя.
    ...
    if request.method == "POST":  # Проверяет, является ли запрос = POST (то есть отправляется ли форма с изменениями)
        ...
        if form.is_valid():  # Дальше все как из прошлой функции, тоесть проверка валидности.
